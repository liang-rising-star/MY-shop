...
const app = createApp({
  setup() {
    ...
    const ucTabs = [{k:'overview',l:'概览'},{k:'orders',l:'订单'},{k:'assets',l:'资产'},{k:'profile',l:'资料'},{k:'security',l:'安全'},{k:'addresses',l:'地址'}]
...
const timedProducts = computed(() => products.value.filter(p => p.type === 'timed'))
    ...
    function statusText(s) { return { pending:'待支付',paid:'已支付',completed:'已完成',refunded:'已退款' }[s] || s }
    ...
    async function loadProfile() {
      ...
      try { user.value = await API.getProfile() } catch(e) { token.value=null; localStorage.removeItem('token') }
    }
    ...
    async function doLogin() {
      authErr.value=''
      try {
        const d=await API.login(authUser.value, authPass.value)
        token.value=d.token; API.token=d.token; localStorage.setItem('token',d.token)
        authUser.value=authPass.value=''; toast('登录成功')
        loadProfile(); loadOrders(); loadMyCoupons()
        navigate(window.location.hash ? 'home' : 'home')
      } catch(e){ authErr.value=e.message }
    }
    async function doReg() {
      authErr.value=''
      try { await API.register(authUser.value, authPass.value, authEmail.value); toast('注册成功，请登录'); isReg.value=false } catch(e){ authErr.value=e.message }
    }
    function logout() {
      token.value=null; API.token=null; localStorage.removeItem('token')
    ...
    async function viewProduct(p) {
      ...
      try { const d=await API.getProduct(p.ID); detail.value=d; loadReviews(p.ID) } catch(e){ toast(e.message,'error') }
      navigate('detail')
    }
    async function checkout(product) {
      if(!token.value){ toast('请先登录','error'); navigate('login'); return }
      try {
        const d=await API.createOrder(product.ID, buyQty.value||1, '')
    ...
    function selectProductForKeys(p) { keyProductID.value=p.ID; adminTab.value='keys' }
    function selectProductForPool(p) {
      const entries=prompt('奖池配置 (商品ID:概率%, 每行一个):\n例如:\n2:50\n3:30\n4:20')
      if(!entries) return
      const data={ entries: entries.trim().split('\n').map(l=>{ const [pid,prob]=l.split(':'); return { prize_id:parseInt(pid), probability:parseFloat(prob) } }) }
      API.updateBlindBoxPool(p.ID,data).then(()=>toast('奖池已更新')).catch(e=>toast(e.message,'error'))
    }
    ...
    async function loadAddresses() { if(!token.value) return; try { addresses.value = await API.request('GET', '/api/addresses') } catch(e){} }
    async function saveAddr() { const f=addrForm.value; if(!f.name||!f.phone) return; try { await API.request('POST', '/api/addresses', f); addrForm.value={name:'',phone:'',region:'',detail:''}; loadAddresses(); toast('地址已保存') } catch(e){ toast(e.message,'error') } }
    async function deleteAddr(id) { try { await API.request('DELETE', '/api/addresses/'+id); loadAddresses(); toast('已删除') } catch(e){ toast(e.message,'error') } }
    async function doRecharge() {
      if (!rechargeAmt.value || rechargeAmt.value <= 0) return
      try { const d = await API.request('POST', '/api/recharge', { amount: rechargeAmt.value }); toast('充值成功'); showRecharge.value = false; rechargeAmt.value = 0; loadUC() }
      catch(e) { toast(e.message, 'error') }
    }
    async function doWithdraw() {
      if (!withdrawAmt.value || withdrawAmt.value <= 0) return
      try { const d = await API.request('POST', '/api/user/withdraw', { amount: withdrawAmt.value }); toast('提现申请已提交'); showWithdraw.value = false; withdrawAmt.value = 0; loadUC() }
      catch(e) { toast(e.message, 'error') }
    }
    ...
    watch(page, (p) => {
      ...
      if (p === 'home' || p === 'lottery' || p === 'events') loadProducts()
      if (p === 'featured') loadProducts(1)
      if (p === 'products') { loadProducts(); loadCategories() }
      if (p === 'orders') loadOrders()
      if (p === 'profile') { loadUC(); loadMyCoupons(); loadUCOrders(); loadAddresses() }
    ...
    function initParticles() {
      ...
      for (let i=0;i<80;i++) particles.push({ x:Math.random()*w, y:Math.random()*h, vx:(Math.random()-0.5)*0.5, vy:(Math.random()-0.5)*0.5, r:Math.random()*2+0.5, alpha:Math.random()*0.5+0.1 })
      function draw() {
        ctx.clearRect(0,0,w,h)
        for(const p of particles){ p.x+=p.vx; p.y+=p.vy; if(p.x<0)p.x=w; if(p.x>w)p.x=0; if(p.y<0)p.y=h; if(p.y>h)p.y=0; ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle=`rgba(0,240,255,${p.alpha})`; ctx.fill() }
        for(let i=0;i<particles.length;i++) for(let j=i+1;j<particles.length;j++){ const dx=particles[i].x-particles[j].x, dy=particles[i].y-particles[j].y, dist=Math.sqrt(dx*dx+dy*dy); if(dist<150){ ctx.beginPath(); ctx.moveTo(particles[i].x,particles[i].y); ctx.lineTo(particles[j].x,particles[j].y); ctx.strokeStyle=`rgba(0,240,255,${0.06*(1-dist/150)})`; ctx.stroke() } }
    ...
    return {
      ...
      authUser, authPass, authEmail, authErr,
      ...
      loadUC, loadUCOrders, updateProfile, changePw, doRecharge, doWithdraw,
      showRecharge, showWithdraw, rechargeAmt, withdrawAmt,
      addresses, addrForm, loadAddresses, saveAddr, deleteAddr,
      ...
      loadProducts, loadCategories, loadOrders, loadAllOrders, loadCoupons, loadProfile, loadMyCoupons, loadCardKeys, loadReviews, loadUsers,
